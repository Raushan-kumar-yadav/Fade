...
// Clip transform (position, scale, rotation, anchor)
...
// Per-effect parameters passed from Python
struct EffectParam {
  std::string typeId;
  struct UniformValue {
    std::string id;
...
// Full frame descriptor sent from Python
  ...
  try {
    ...
    for (const auto &c : j.value("clips", nlohmann::json::array())) {
      ...
      if (cd.type == ClipDesc::Type::Text && c.contains("textStyle")) {
        ...
        auto shc =
            s.value("shadowColor", nlohmann::json::array({0, 0, 0, 0.6}));
      ...
      if (cd.type == ClipDesc::Type::Shape && c.contains("shapeStyle")) {
        ...
        auto shc =
            s.value("shadowColor", nlohmann::json::array({0, 0, 0, 0.75}));
