...
static sk_sp<SkTypeface> resolveTypeface(const std::string &family, bool bold, bool italic) {
  ...
  if (!tf && name != "Arial") tf = mgr->matchFamilyStyle("Arial", style);
...
// Easing functions — 0=linear, 1=ease_in, 2=ease_out, 3=ease_both
static float applyEasing(float t, int easing) {
  t = std::max(0.f, std::min(1.f, t));
  switch (easing) {
    case 1: return t * t;                        // ease_in
    case 2: return t * (2.f - t);               // ease_out
    case 3: return t < 0.5f                     // ease_both (smooth-step)
                   ? 2.f * t * t
                   : -1.f + (4.f - 2.f * t) * t;
...
// Split UTF-8 string into individual characters (simple ASCII + multi-byte passthrough)
...
// Draw a single string unit with per-unit animator value applied
static void drawUnit(SkCanvas *canvas, const std::string &unit,
                     float x, float y, float opacity, float offX, float offY, float scale,
                     const SkFont &font, const ClipDesc::TextDesc &ts, float clipOpacity) {
  canvas->save();
  canvas->translate(x + offX, y + offY);
  if (scale != 1.f) canvas->scale(scale, scale);

  const float finalAlpha = clipOpacity * opacity;

  if (ts.shadowEnabled && ts.shadowBlur > 0.f) {
    SkPaint sp;
    sp.setColor(toSkColor(ts.shadowR, ts.shadowG, ts.shadowB, ts.shadowA * finalAlpha));
    sp.setAntiAlias(true);
    sp.setMaskFilter(SkMaskFilter::MakeBlur(kNormal_SkBlurStyle, ts.shadowBlur * 0.5f));
    canvas->drawString(unit.c_str(), ts.shadowOffsetX - offX, ts.shadowOffsetY - offY + (scale != 1.f ? (y * (1.f - scale)) : 0.f), font, sp);
    // Simpler: draw shadow at offset from local origin
    sp.setMaskFilter(SkMaskFilter::MakeBlur(kNormal_SkBlurStyle, ts.shadowBlur * 0.5f));
    canvas->drawString(unit.c_str(), ts.shadowOffsetX, ts.shadowOffsetY, font, sp);
  }
  ...
  if (ts.strokeWidth > 0.f) {
    ...
    stroke.setColor(toSkColor(ts.strokeR, ts.strokeG, ts.strokeB, ts.strokeA * finalAlpha));
...
void drawText(SkCanvas *canvas, const ClipDesc &clip, int canvasW, int canvasH) {
  ...
  const float originY = static_cast<float>(canvasH) * 0.5f - totalH * 0.5f + ts.fontSize;
  ...
  if (ts.bgEnabled) {
    ...
    if (ts.bgCornerRadius > 0.f)
      canvas->drawRoundRect(SkRect::MakeXYWH(bx, by, bw, bh), ts.bgCornerRadius, ts.bgCornerRadius, bgp);
    else
      canvas->drawRect(SkRect::MakeXYWH(bx, by, bw, bh), bgp);
  }

  // Precompute all units for the animator
  // Build a flat list: (lineIdx, unitStr) for chars/words/lines mode
  const bool useAnimator = ts.animEnabled;

  // Collect all units across all lines for total count
  struct UnitInfo { int lineIdx; std::string text; float preX; float preY; };
  std::vector<UnitInfo> allUnits;

  if (useAnimator && ts.animMode < 2) {
    // char or word mode — flatten all lines
    for (int li = 0; li < (int)lines.size(); ++li) {
      const std::string &ln = lines[li];
      if (ln.empty()) continue;
      float lineW = font.measureText(ln.c_str(), ln.size(), SkTextEncoding::kUTF8);
      ...
      std::vector<std::string> units = (ts.animMode == 1) ? splitWords(ln) : splitChars(ln);
      float cx2 = lx;
      for (auto &u : units) {
  ...
  for (int li = 0; li < (int)lines.size(); ++li) {
    ...
    const float lineW = font.measureText(ln.c_str(), ln.size(), SkTextEncoding::kUTF8);
    float lx = (ts.alignment == "center") ? originX - lineW * 0.5f
             : (ts.alignment == "right")  ? originX - lineW
             :                              originX - maxLineW * 0.5f;
    ...
    if (!useAnimator) {
      ...
      if (ts.shadowEnabled && ts.shadowBlur > 0.f) {
        SkPaint sp;
        sp.setColor(toSkColor(ts.shadowR, ts.shadowG, ts.shadowB, ts.shadowA * opacity));
        sp.setAntiAlias(true);
        sp.setMaskFilter(SkMaskFilter::MakeBlur(kNormal_SkBlurStyle, ts.shadowBlur * 0.5f));
        canvas->drawString(ln.c_str(), lx + ts.shadowOffsetX, ly + ts.shadowOffsetY, font, sp);
      }
      if (ts.strokeWidth > 0.f) {
        SkPaint sp;
        sp.setStyle(SkPaint::kStroke_Style);
        sp.setStrokeWidth(ts.strokeWidth);
        sp.setColor(toSkColor(ts.strokeR, ts.strokeG, ts.strokeB, ts.strokeA * opacity));
        sp.setAntiAlias(true);
      ...
      switch (ts.animProperty) {
        case 0: aOpacity = std::max(0.f, std::min(1.f, val)); break;
        case 1: aOffY    = val; break;
        case 2: aOffX    = val; break;
        case 3: aScale   = val; break;
      }
      drawUnit(canvas, ln, lx, ly, aOpacity, aOffX, aOffY, aScale, font, ts, opacity);

    } else {
      // Char/word mode — draw each unit separately with its own animated value
      // Find units for this line in allUnits
      const int total = (int)allUnits.size();
      float cx2 = lx;
      std::vector<std::string> lineUnits = (ts.animMode == 1) ? splitWords(ln) : splitChars(ln);
      // Find global start index for this line
      int unitBase = 0;
      for (auto &u : allUnits) {
        if (u.lineIdx == li) break;
        ++unitBase;
      }

      for (int ui = 0; ui < (int)lineUnits.size(); ++ui) {
        const std::string &unit = lineUnits[ui];
        float uw = font.measureText(unit.c_str(), unit.size(), SkTextEncoding::kUTF8);
        ...
        drawUnit(canvas, unit, cx2, ly, aOpacity, aOffX, aOffY, aScale, font, ts, opacity);
        cx2 += uw + ts.letterSpacing;
      }
    }
  }
