...
HWVideoDecoder::get_format_callback(AVCodecContext *ctx,
  ...
  for (const AVPixelFormat *p = pix_fmts; *p != -1; p++) {
    if (*p == AV_PIX_FMT_D3D11) {
      ...
      if (!logged) {
        LOG_INFO("FFmpeg requested D3D11 format - Handshake SUCCESSFUL");
        logged = true;
      }
      return *p;
    }
...
std::shared_ptr<DecodedFrame> HWVideoDecoder::decodeFrame(int64_t targetFrame) {
  ...
  if (needSeek) {
    std::cout << "[DECODE] SEEK  target=" << targetFrame
              << " last=" << m_lastDecodedFrame << " gap=" << gap << "\n"
              << std::flush;
    double timeInSecond = static_cast<double>(targetFrame) / m_fps;
    int64_t target_pts =
        static_cast<int64_t>(std::round(timeInSecond / av_q2d(m_timeBase)));
  ...
  while (true) {
    ...
    if (response == 0) {
      ...
      if (frame->format == AV_PIX_FMT_D3D11) {
        ...
        int dstW = std::max(2, static_cast<int>(srcW * m_previewScale)) &
                   ~1; // ensure even
        ...
        if (!m_swsCtx || m_srcWidth != srcW || m_srcHeight != srcH ||
          ...
          LOG_INFO("[HWVideoDecoder] Preview scale: " + std::to_string(srcW) +
                   "x" + std::to_string(srcH) + " -> " + std::to_string(dstW) +
                   "x" + std::to_string(dstH));
        }
      ...
      double decMs = std::chrono::duration_cast<std::chrono::microseconds>(
      ...
      if (current_frame_idx != targetFrame) {
        ...
        std::cout << "[DECODE] OK frame=" << targetFrame << " dt=" << decMs
                  << "ms\n"
                  << std::flush;
      }
      break;
  ...
  while (true) {
    ...
    if (response == 0) {
      ...
      double decMs = std::chrono::duration_cast<std::chrono::microseconds>(
                         ...
                         .count() /
                     1000.0;
      std::cout << "[DECODE-D] OK frame=" << targetFrame << " dt=" << decMs
                << "ms\n"
                << std::flush;
