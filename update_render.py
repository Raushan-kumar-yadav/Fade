...
pattern1 = re.compile(r'            if ic_type == "image" and hasattr\(inner_clip, "brush_strokes"\) and inner_clip\.brush_strokes:\n(.*?)\n                    inner_clips\.append\(brush_clip\)', re.DOTALL)

def replace1(match):
    return """            if hasattr(inner_clip, "brush_strokes") and inner_clip.brush_strokes:
...
print(f'Replaced {n1} instances in first block')
